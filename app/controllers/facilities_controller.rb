class FacilitiesController < ApplicationController
  ...
  def index
    ...
    @facilities = Facility.includes(:bookings)
    

    if params[:sport].present?
      @facilities = @facilities.where(sport: params[:sport])
    end

    if params[:pincode].present?
      @facilities = @facilities.where(pincode: params[:pincode])
    end
  end
  ...
  def tennis
    @tennis_facilities = Facility.where(sport: 'tennis')
  end
  ...
  def facility_params
    params.require(:facility).permit(:name, :description, :location, :pincode, :sport)
  end
end
